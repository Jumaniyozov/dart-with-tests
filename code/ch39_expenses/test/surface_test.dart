...
void main() {
  // #region surface
  /// Every library `package:ch39_expenses/expenses.dart` offers.
  ///
  ...
  // #region runnable
  /// Every program `dart run ch39_expenses:<name>` will start.
  ///
  ...
  group('bin is a contract too', () {
    test('and it offers exactly the three programs it says it does', () {
      /// Still three, and two of the three names moved. `holding.dart` was a
      /// comparison artifact and study 39 is what it was compared against, so
      /// it goes the way `by_hand.dart` went at study 36; `migrate.dart` is the
      /// program that moves the reader's file into the database, once.
      /// Both halves are breaking changes and the count hiding that is exactly
      /// why this list holds names rather than a number.
      const runnable = {'expenses.dart', 'migrate.dart', 'serve.dart'};
