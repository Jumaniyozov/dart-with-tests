...
Future<Outcome> run(
  ...
  Store store,
  Day today, {
  Alone alone = unguarded,
}) async {
  // **[alone] is named and defaulted where `Tracker`'s is positional and
  // required, and the asymmetry is deliberate.** `Tracker` is the layer the
  // boundary is spoken about at, so it has to be said there. This is one
  // caller of it, and every test that calls `run` has one store and one
  // caller, which is the configuration `unguarded` describes exactly. `bin/expenses.dart` passes the real one, because two
  // terminals are two writers.
  //
  // A `Day` in and a clock out, and the mismatch is the point. `Tracker` wants
  // to be asked what day it is, because study 38 found a caller that runs for
  // days. This one does not: a command reads the clock once, in `bin/`, and is
  // finished before it could be wrong. So the edge that has a day hands over a
  // function that keeps answering it, and `test/command_test.dart` is not
  // edited for a third study running.
...
Future<Outcome> _list(Tracker tracker, Period? period) async {
  final recorded = await tracker.expenses(period: period);
  if (recorded.isEmpty) {
    return (
      code: okay,
      out: period == null
