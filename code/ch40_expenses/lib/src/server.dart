...
///
/// **Two callers could both get past that check, and study 37 said so here.**
/// That sentence named `FileStore`, which stopped being what `bin/serve.dart`
/// builds at study 39 — so it was describing a program this package no longer
/// runs for one whole study, on a page, with a green suite underneath it.
///
/// What is true now is narrower and is `Tracker`'s to state rather than this
/// route's: [Tracker.record] reads, decides and writes inside the `Alone` it
/// was built with, and `bin/serve.dart` builds it with one that takes the
/// database's write lock first. Nothing here changed, which is the point —
/// a route that writes should not be where a transaction boundary is
/// discovered.
