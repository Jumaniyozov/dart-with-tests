...
A major version for a third argument. A `Tracker` is built from a store, a
clock and a transaction, and the third one has no default — because the
guarantee study 39 shipped was one nobody had asked for and nobody had written
down.

- **Changed** — `Tracker(Store store, Day Function() today, Alone alone)`. The
  read, the decision and the write in `Tracker.record` now happen inside
  whatever the edge handed over. Measured: with `unguarded` and two calls in
  flight, two £6.00 lunches are both recorded against a £10.00 limit; with the
  one `bin/serve.dart` hands over, the second caller is refused.
- **Added** — `Alone` and `unguarded`, both exported. `Alone` is a generic
  function type and not a member on `Store`, because study 28 settled that an
  interface promises what its weakest implementation can keep, and `FileStore`
  cannot keep this one.
