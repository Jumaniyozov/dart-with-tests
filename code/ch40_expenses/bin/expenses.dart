...
/// `expenses.txt` to `expenses.db`. `dart run ch40_expenses:migrate` is the one
...
Future<void> main(List<String> args) async {
  ...
  final outcome = await run(
    ...
    SqliteStore(db),
    Day.on(DateTime.now()),
    // **This program is the second writer, and it always was.** Study 38 kept
    // the command line so a reader could run it in another terminal against
    // the file the server was holding; study 40 is where that stops being a
    // demonstration and starts being a thing the program has to survive. A
    // A generic function literal, because `Alone` is a generic function type
    // and `aloneIn` needs the database this program opened three lines up.
    alone: <T>(body) => aloneIn(db, body),
  );
  db.close();
