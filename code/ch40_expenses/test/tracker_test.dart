...
void main() {
  ...
  setUp(() {
    store = InMemoryStore();
    tracker = Tracker(store, () => today, unguarded);
  });
  // #endregion setup
  ...
  group('the use cases, with no terminal anywhere', () {
    test('record keeps an expense and says nothing stopped it', () async {
      ...
      expect((await store.expenses()).single.note, 'coffee');
    });
    ...
    test('expenses narrows to a period when it is given one', () async {
      ...
      final september = await tracker.expenses(period: Period.of(today));
  ...
  group('what the extraction actually moved', () {
    ...
    test('every private use case took a Store and a Day, and none does now', () {
      // Whitespace collapsed before anything is matched. Study 40 made `run`'s
      // signature long enough for `dart format` to wrap it, and the first
      // version of this test asserted the one-line spelling — which made it a
      // test about the formatter.
      final flat = code().join(' ').replaceAll(RegExp(r'\s+'), ' ');

      expect(RegExp(r'\bStore\b').allMatches(flat), hasLength(1));
      expect(RegExp(r'\bDay\b').allMatches(flat), hasLength(1));
      expect(
        flat,
        contains(
          'Future<Outcome> run( List<String> args, Store store, Day today, '
          '{ Alone alone = unguarded, }) async {',
        ),
        reason:
            ...
            'place left that holds either — joined at study 40 by a third, '
            'which is the one thing about a Tracker a caller can get wrong',
      );
    });
  ...
  group('the day is asked for now, not remembered from then', () {
    ...
    test('and the command line still reads its clock once, in bin/', () {
      ...
      expect(
        ...
        'final tracker = Tracker(store, () => today, alone);',
        reason:
            ...
            'that keeps answering it — and hands over its transaction the same '
            'way, which is the only thing study 40 changed in this line',
      );
    });
  });
  // #endregion clock
