...
// #region counting
/// A [Store] holding a thousand expenses, counting what being asked costs.
///
...
/// to answer it.
///
/// **It is study 37's double with one member renamed, and the number it
/// reports has changed.** What it measures is the boundary: [handedOver] is how
/// many expenses crossed from the store to the edge. It still builds a thousand
/// on every call, and that is not a dodge — a double stands for a store that
/// can keep a bound, and whether one really can is a question about
/// `SqliteStore`'s `LIMIT`, asked in `test/sqlite_store_test.dart`.
class CountingStore implements Store {
  ...
  Future<List<Expense>> expenses({Period? period, int? count}) async {
    ...
    final wanted = [
      for (final expense in held)
        if (period == null || period.contains(expense.day)) expense,
    ];
    final bounded = count == null ? wanted : wanted.take(count).toList();
    handedOver += bounded.length;
    return bounded;
  }
...
void main() {
  ...
  group("study 26's taxonomy, with a wire under it", () {
    ...
    test('an acknowledged breach is 200 and is kept', () async {
      ...
      expect(response.statusCode, 200);
      expect(await store.expenses(), hasLength(3));
    });
  ...
  group('what the edge files an expense under', () {
    ...
    test('is the day the clock answers when the request arrives', () async {
      ...
      expect(
        [for (final expense in await store.expenses()) expense.day],
  ...
  // #region bound
  group('the bound reaches the store, and here is the measurement', () {
    test('one expense and a thousand no longer cost the same', () async {
      ...
      expect([small.reads, small.handedOver], [1, 1]);
      expect(
        [large.reads, large.handedOver],
        [1, 1000],
        reason:
            'study 37 measured 1000 in both rows: the document got a '
            'thousand times smaller and not one read moved. The bound is an '
            'argument now.',
      );
    });

    test(
      'and a month reaches it too, rather than being sieved out here',
      () async {
        final store = CountingStore();

        await ask(store, 'GET', '/expenses?month=2026-10&limit=5');

        expect(store.asked, 5);
        expect(
          store.handedOver,
          0,
          reason:
              'every expense this double holds is in September, so October is '
              'empty — and the store is the thing that knows that now',
        );
      },
    );

    test('no query string asks for no bound at all', () async {
      final store = CountingStore();

      await ask(store, 'GET', '/expenses');

      expect(store.asked, isNull);
      expect(store.handedOver, 1000);
    });
  });
  // #endregion bound
    ...
    test(
      ...
      () {
        ...
        expect(
          ...
          {'file_store.dart'},
          reason:
              "that file's own doc comment says so, and two studies running "
              'could have made it a lie without either test above noticing — '
              'a HoldingStore taking a File at 38, and a SqliteStore opening '
              'its own path at 39. Both take what they need as a parameter, '
  ...
  group('shelf is dart:io plus a function type', () {
    test('so serve hands back the dart:io server it bound', () async {
      final server = await io.serve(
        expensesApi(
          Tracker(InMemoryStore(), () => today, unguarded),
          key: key,
          report: (error, stack) {},
        ),
        'localhost',
