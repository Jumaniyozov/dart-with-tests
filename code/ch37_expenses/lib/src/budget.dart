...
/// It does not fit, this is by how much, and this is the limit it broke.
///
/// **The [Limit] is study 37's, and it was free.** Study 36 shipped a `Breach`
/// carrying only `over`, so the command line had to go back to the store for
/// the limit's amount before it could write *budgeted at £20.00* — a second
/// read, on the refusal path, for a fact the decision had been holding all
/// along. [Budget.on] has the limit in scope when it builds this; handing it
/// over costs nothing and saves the only caller that wanted it a round trip.
///
/// It is the whole [Limit] rather than its amount because the category comes
/// with it, and an API answering *which* budget refused you needs both. A
/// second `Money` beside `over` would also have been two parameters of one
/// type, in a positional constructor, waiting to be swapped.
///
/// Adding it is a **breaking change**, and there is no way for it not to be:
/// the field list of a primary constructor is its parameter list, so a new
/// field is a new required argument at every place one is built. Hence `3.0.0`.
final class const Breach(final Money over, final Limit limit) extends Verdict {}
// #endregion verdict
...
class const Budget._(
  ...
  Verdict on(Expense expense) {
    ...
    // the two, so this subtraction is the one that goes. `limit` is the field
    // this budget was built from — the verdict is told which limit it broke by
    // the only object that ever knew.
    return Breach((after - limit.amount)!, limit);
  }
