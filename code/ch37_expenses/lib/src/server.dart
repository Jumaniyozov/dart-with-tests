...
///
/// **`limit` here is a lie and the study says so.** Slicing after
/// [Tracker.expenses] has answered means the store has already read every
/// expense there is and this throws most of them away: the document gets
/// smaller and the work does not. `test/server_test.dart#bound` measures
/// exactly that with a store that counts what it hands over, and study 39 is
/// where a bound can be pushed down to something that can keep it.
///
...
Future<Response> _list(Tracker tracker, Request request) async {
  ...
  final expenses = await tracker.expenses(period);
  return _json(200, [
    for (final expense in bound == null ? expenses : expenses.take(bound))
      expense.toJson(),
  ]);
}
...
/// caller's, and it is [Tracker.today] for exactly as long as study 38 takes to
/// notice that a server does not exit.
