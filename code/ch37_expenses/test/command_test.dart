...
void main() {
  ...
  group('and the program never refuses to write down what was spent', () {
    ...
    test('--anyway records the overspend, and marks it', () async {
      ...
      expect((await store.all).last.acknowledged, isTrue);
      expect((await store.all).first.acknowledged, isFalse);
    });
  ...
  group('a run that did not', () {
    ...
    test(
      ...
      () async {
        ...
        expect(await store.all, isEmpty, reason: 'neither one was recorded');
      },
    );
    // #endregion dash
  });
