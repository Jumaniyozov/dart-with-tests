...
class SpyStore implements Store {
  ...
  @override
  void record(Expense expense) => calls.add('record');
...
void main() {
  ...
  group('a mock knows how it was called', () {
    test('and can say record happened exactly once', () {
      final spy = SpyStore();
      run(['add', '12.50', 'food', 'coffee'], spy, day);
      expect(spy.calls, ['record']);
    });

    test('and cannot say what was recorded', () {
      final spy = SpyStore();
      run(['add', '12.50', 'food', 'coffee'], spy, day);
      expect(
        run(['list'], spy, day).out,
        'nothing recorded yet',
        reason: 'it counted the call and kept nothing, so list has nothing',
      );
    });

    test('because its ledger is a record of the implementation', () {
      final spy = SpyStore();
      run(['add', '12.50', 'food', 'coffee'], spy, day);
      run(['list'], spy, day);
      expect(
  ...
  group('a fake knows what happened', () {
    ...
    test('so a test can ask what the program did, not how it did it', () {
      run(['add', '12.50', 'food', 'coffee'], store, day);
      final listed = run(['list'], store, day).out;
  ...
  group('the interface is the only thing run knows about', () {
    test('a store that keeps only the last expense works unchanged', () {
      final store = LastOnlyStore();
      run(['add', '1.00', 'food', 'apple'], store, day);
      run(['add', '2.00', 'food', 'soup'], store, day);
      expect(store.all.single.note, 'soup');
      expect(run(['list'], store, day).out, contains('food: £2.00'));
    });
  });

  group('totals is derived, so every store answers it the same way', () {
    test('including one that never stores anything', () {
      expect(SpyStore().totals, isEmpty);
    });

    test('and one that keeps a single expense', () {
      final store = LastOnlyStore()
        ..record(Expense(Money.fromPence(250), Category('food'), day, 'soup'));
      expect(store.totals, {Category('food'): Money.fromPence(250)});
    });
  });
}
...
class LastOnlyStore implements Store {
  ...
  @override
  void record(Expense expense) => _last = expense;

  @override
  List<Expense> get all => [?_last];
}
