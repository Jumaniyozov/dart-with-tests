...
void main() {
  ...
  group('the verdict on one more expense', () {
    ...
    test('and which limit it was that it broke', () {
      final verdict = budgetOver([spent(15000)]).on(spent(6000));

      expect(
        (verdict as Breach).limit,
        food,
        reason:
            'the budget had the limit in hand when it decided; study 36 made '
            'both callers go back to the store for it afterwards',
      );
    });
  ...
  group('what stands in the way of an expense being recorded', () {
    test('a breach nobody acknowledged does', () {
      expect(Breach(Money.fromPence(500), food).refuses(spent(2500)), isTrue);
    });
    ...
    test('and the same breach does not, once the expense says so', () {
      ...
      expect(Breach(Money.fromPence(500), food).refuses(anyway), isFalse);
    });
