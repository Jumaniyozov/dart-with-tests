...
void main() {
  ...
  group('what an assert is worth at run time', () {
    test('it fires here, because dart test turns asserts on', () {
      expect(half(8), 4);
      expect(
        () => half(7),
        throwsA(
          isA<AssertionError>().having(
            (e) => e.message,
            'message',
            'half wants an even number',
          ),
        ),
      );
    });

    test('and that is the only place in this repository where it does', () {
      // `dart run` and a binary from `dart compile exe` both run with asserts
      // off. 26.4 has the transcripts. This test is the control.
      expect(half(8), 4);
    });
  });
}
